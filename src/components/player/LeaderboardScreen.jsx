...
export default function LeaderboardScreen({ isFinal }) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 max-w-md mx-auto relative overflow-hidden">
      {showConfetti && (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex-1">
        <div className="text-center mb-6">
          ...
          {!isFinal && (
            <p className="text-gray-400 text-sm mt-1">Siguiente pregunta próximamente...</p>
          )}
        </div>

        {/* Own score highlight */}
        <div className="glass-card-glow p-4 text-center mb-6">
        ...
        <div className="glass-card p-4 space-y-2">
          {leaderboard.map((player, idx) => (
            ...
            >
              ...
              <div className={`player-avatar ${AVATAR_COLORS[idx % AVATAR_COLORS.length]}`} style={{ width: 32, height: 32, fontSize: 13 }}>
        ...
        {isFinal && (
          <button className="btn-secondary w-full mt-6" onClick={leaveSession}>
            🏠 Salir
          </button>
        )}
      </motion.div>
